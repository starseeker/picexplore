...
int read_pgm(const char *filename, unsigned char *&data, int &w, int &h)
{
    ...
    if (fscanf(in_file, "%d", &w) != 1 ||
        fscanf(in_file, "%d", &h) != 1 ||
        fscanf(in_file, "%d", &i) != 1) {
        fclose(in_file);
        return -1;
    }
    while (getc(in_file) != '\n')
    ...
    if (fread(_data, sizeof(unsigned char), (w) * (h), in_file) != (size_t)((w) * (h))) {
        free(_data);
        fclose(in_file);
        return -1;
    }
...
int read_ppm(const char *filename, unsigned char *&data, int &w, int &h)
{
    ...
    if (fscanf(fp, "%4d", &width) != 1 ||
        fscanf(fp, "%4d", &height) != 1 ||
        fscanf(fp, "%3d", &maxComp) != 1) {
        fclose(fp);
        return -1;
    }
    if (fread(cookie, 1, 1, fp) != 1) {
        fclose(fp);
        return -1;
    }
