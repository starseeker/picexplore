        ...
        switch (event) {
            case FL_MOUSEWHEEL: {
                ...
                if (zoom_ == 0.0f) {
                    ...
                    if (full_res_w_ == 0 || full_res_h_ == 0) return 1;
                    float scale_x = (float)w() / full_res_w_;
                    float scale_y = (float)h() / full_res_h_;
                    zoom_ = std::min(scale_x, scale_y);
                    pan_x_ = (w() - full_res_w_ * zoom_) / 2.0f;
                    pan_y_ = (h() - full_res_h_ * zoom_) / 2.0f;
                }
